...
if [ ! -e $TOGGLE ]; then
    ...
    notify-send 'Auto-Suspend' 'Off' -i /usr/share/icons/gnome/48x48/status/changes-allow.png
else
    rm -f $TOGGLE
    /usr/bin/xautolock -time 30 -locker "systemctl suspend" -detectsleep &
    notify-send 'Auto-Suspend' 'On' -i /usr/share/icons/gnome/48x48/status/changes-prevent.png
fi
